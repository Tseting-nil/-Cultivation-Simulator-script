...
local units = {"Sec Gold", "Min Gold", "Hour Gold", "Day Gold"} -- 切換單位的選項
...
local function calculateGPS()
    ...
    if currentUnitIndex == 1 then
        ...
        if gpsDisplay >= 10000000 then
            ...
            unitLabel = string.format("%d (M)gold/sec", gpsDisplay)
        elseif gpsDisplay >= 10000 then
            gpsDisplay = math.floor(gpsDisplay / 1000)  -- K
            unitLabel = string.format("%d (k)gold/sec", gpsDisplay)
        else
            unitLabel = string.format("%d gold/sec", gpsDisplay)
        end
    elseif currentUnitIndex == 2 then
        gpsDisplay = math.floor(avgGPS * 60)  -- 每分鐘的 GPS
        if gpsDisplay >= 10000000 then
            gpsDisplay = math.floor(gpsDisplay / 1000000)  -- M
            unitLabel = string.format("%d (M)gold/min", gpsDisplay)
        elseif gpsDisplay >= 10000 then
            gpsDisplay = math.floor(gpsDisplay / 1000)  -- K
            unitLabel = string.format("%d (k)gold/min", gpsDisplay)
        else
            unitLabel = string.format("%d gold/min", gpsDisplay)
        end
    elseif currentUnitIndex == 3 then
        gpsDisplay = math.floor(avgGPS * 3600)  -- 每小時的 GPS
        if gpsDisplay >= 10000000 then
            gpsDisplay = math.floor(gpsDisplay / 1000000)  -- M
            unitLabel = string.format("%d (M)gold/hour", gpsDisplay)
        elseif gpsDisplay >= 10000 then
            gpsDisplay = math.floor(gpsDisplay / 1000)  -- K
            unitLabel = string.format("%d (k)gold/hour", gpsDisplay)
        else
            unitLabel = string.format("%d gold/hour", gpsDisplay)
        end
    ...
    elseif currentUnitIndex == 4 then
        ...
        if gpsDisplay >= 10000000 then
            gpsDisplay = math.floor(gpsDisplay / 1000000)  -- M
            unitLabel = string.format("%d (M)gold/day", gpsDisplay)
        elseif gpsDisplay >= 10000 then
            gpsDisplay = math.floor(gpsDisplay / 1000)  -- K
            unitLabel = string.format("%d (k)gold/day", gpsDisplay)
        else
            unitLabel = string.format("%d gold/day", gpsDisplay)
        end
